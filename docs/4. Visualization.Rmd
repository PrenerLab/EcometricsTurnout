...
ggplot() +
  geom_sf(data = bg) +
  geom_sf(data = m_all, mapping = aes(fill = csb)) +
  ...
  scale_fill_viridis(name = "CSB Calls", guide = guide_colorbar(ticks = F), direction = -1) +
  facet_grid(. ~ year)
...
ggplot() +
  ...
  theme(axis.text = element_blank(),
        ...
        plot.background = element_rect(fill = "transparent", color = NA),
        legend.background = element_rect(fill = "transparent", color = NA)) +
  scale_fill_viridis(option = "plasma", name = "Voter Turnout", guide = guide_colorbar(ticks = F), direction = -1) +
  facet_grid(. ~ year)
